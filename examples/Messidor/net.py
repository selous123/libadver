...
class VGG_s(nn.Module):
    def __init__(self):
        ...
        self.features = self._make_fea_layers()
    ...
    def _make_fea_layers(self):
        ...
        layers += [nn.Conv2d(96, 256, kernel_size = 5, stride = 1),
                nn.BatchNorm2d(256),
                nn.ReLU(inplace = True)]

        layers += [nn.MaxPool2d(kernel_size = 2, stride = 2)]

        layers += [nn.Conv2d(256, 512, kernel_size = 3, stride = 1),
                nn.BatchNorm2d(512),
                nn.ReLU(inplace = True),
